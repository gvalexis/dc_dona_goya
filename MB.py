...
data=pd.read_csv("entregable.csv")
data['variable'].value_counts()
data=data.pivot(index="id_ventas",columns="nventa",values="variable")
data
for i in range(0,13):
 data.iloc[:,i]=data.iloc[:,i].replace({np.nan:''})

#corremos el algoritmo de reglas de asociacion 
transactions =[]
for i in range(0,10229):
    transactions.append([str(data.values[i,j]) for j in range(1,13)])
rules=apriori(transactions,min_support=0.017,min_confidence=.60,min_lift =1 ,min_length=3)
results=list(rules)

frame = pd.DataFrame(inspect(results),columns=['recomendaciones','compra','soporte','confianza','lift'])
k=frame.iloc[0,1]
frame=frame[frame['compra']!=k]
frame=frame[frame['recomendaciones']!=k]

frame.to_excel("reglas.xlsx")
